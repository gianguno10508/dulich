...
class TraveModule
{
    ...
    public function editTravel($name,$post,$code,$date,$time,$startingplace,$ToLocation,$Numberofseats,$getid)
    {
        ...
        try{
            $sql = "UPDATE travelviewing  SET title = '$name', content = '$post',code= '$code', time = '$date',Dday = $time,startingplace = '$startingplace',ToLocation = '$ToLocation',Numberofseats = $Numberofseats where  id=$getid";
            mysqli_query($this->conect, $sql);
            return true;
